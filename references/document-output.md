...
- 无提交时 `source_commit: unavailable` 并说明原因；未判定等级写 `pending`。
- 时间使用带时区的 ISO 8601。
- 增加 `execution_mode: autonomous_ai`、`executor` 和 `verifier`；外部授权适用时记录 `authorization_scope` 与证据，不适用时写 `not_required`。

## 内容与证据

包含结论、已验证证据、待验证假设、风险、交付物和 AI 后续动作；复用当前阶段模板，省略空章节。记录相对文件路径、精确命令、退出码和必要结果，不复制长日志或写入密钥、个人信息和生产数据。
...
| 事故 | 影响、时间线、根因、恢复、机制缺口及带 AI 执行单元、目标时间和验证条件的改进项 |
| 成熟度 | 各能力证据和评分、当前等级、P0/P1 与分阶段改进路线 |

P0/P1 未关闭时设为 `blocked`，记录 AI 执行单元、关闭条件和验证方式；缺少执行上下文时由编排器创建执行单元。重大结论变化需记录日期、原结论和原因。代码或测试变化后同步更新受影响文档；最终列出更新路径、结论、阻断项和未执行阶段。
